...
class Casino(torch.nn.Module):
    ...
    def fair_roll(self, n_roll=1):
        ...
        return torch.randint(low=1, high=7, size=(n_roll,), device=self.device, requires_grad=False)

    def loaded_roll(self, n_roll=1):
        # NB 50% of a 6, 1/10 otherwise.
        draws = torch.randint(low=1, high=11, size=(n_roll,), device=self.device, requires_grad=False)
        draws[draws > 6] = 6

        return draws

    def sample(self, n_seq=1):
        is_loaded = torch.randint(low=0, high=2, size=(n_seq,), device=self.device, requires_grad=False)
    ...
    def emission(self, state, obvs, log=True):
        if state is None:
            state = torch.tensor(
                range(self.n_states), device=self.device, dtype=torch.int32,
            )

        n_states = len(state)

        # NB there is no emission from a bookend state
        result = -99.0 * torch.ones((n_states, self.n_obvs), device=self.device)
        state = state.unsqueeze(-1) * torch.ones_like(
            result, dtype=torch.int32, device=self.device
        )

        result[(state == 1)] = torch.tensor([1.0 / 6.0], device=self.device)
        result[(state == 2) & (obvs == 6)] = torch.tensor(
            [1.0 / 2.0], device=self.device
        )
        result[(state == 2) & (obvs != 6)] = torch.tensor(
            [1.0 / 10.0], device=self.device
        )
